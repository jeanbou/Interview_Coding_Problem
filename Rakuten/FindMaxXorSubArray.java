public class FindMaxXorSubArray { // Assumed int size
	...
	static int query(int pre_xor) {
		...
		for (int i=INT_SIZE-1; i>=0; i--) {	// Find current bit in given prefix
			...
			// bit, then look for same bit. else
			if (temp.arr[val] != null) {
				temp = temp.arr[val];
			}
		}
	...
	// arr[0..n-1]
	static int maxSubarrayXOR(int arr[], int n) { // Create a Trie and insert 0 into it
		root = new TrieNode();
		insert(0); // Initialize answer and xor of current prefix
		int result = Integer.MIN_VALUE;
		int pre_xor =0;
	
		// Traverse all input array element
		for (int i=0; i<n; i++) { // update current prefix xor and insert it 
			// into Trie
			pre_xor = pre_xor^arr[i];
			insert(pre_xor); // Query for current prefix xor in Trie and
